...
- [x] T013 [artifacts: ui] Extend Settings with AI provider and model controls: provider selection for Claude/Codex, provider-specific model inputs, persistence through the new API, defaults that show existing Claude behavior, and no loss of inactive provider model values when switching.
