...
test('open a GitLab MR via REST and submit a comment review', async ({ page }) => {
  const errors: string[] = [];
  ...
  // Diff content from the mock REST API should be visible
...
test('open a GitLab MR via REST and submit an approval', async ({ page }) => {
  await page.goto('/');
